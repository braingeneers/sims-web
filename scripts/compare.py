...
if __name__ == "__main__":
    ...
    # print_diffs("encodings", o_enc[1], s_enc[0][1].detach().numpy(), 5)
    # print_diffs("encodings", o_enc[2], s_enc[0][2].detach().numpy(), 5)
    ...
    # Compare
    print_diffs(
        "first encoder", steps_output_py[0].detach().numpy(), steps_output_onnx[0], 5
    )
    print_diffs(
        "second encoder", steps_output_py[1].detach().numpy(), steps_output_onnx[1], 5
    )
    print_diffs(
        "third encoder", steps_output_py[2].detach().numpy(), steps_output_onnx[2], 5
    )

    # !!!! But in batches > 1 size some samples values don't match...WHY?!!!
    print_diffs(
        "encodings",
        steps_output_py[1][0:-1].detach().numpy(),
        steps_output_onnx[1][0:-1],
        5,
    )
    print_diffs(
        "encodings",
        steps_output_py[1][-1].detach().numpy(),
        steps_output_onnx[1][-1],
        5,
    )
    ...
    print_diffs("res", res_py.detach().numpy(), res_onnx[0], 5)

    out_py = sims.model.network.tabnet.final_mapping(res_py)
    out_onnx = session.run(None, {"input": res_onnx})
    print_diffs("mappings", out_py.detach().numpy(), out_onnx, 5)
    ...
    torch.onnx.export(
        ...
        torch.zeros(1, len(sims.model.genes)),
        logits_path,
        opset_version=17,  # 12 works in web runtime, later doesn't
        do_constant_folding=True,  # whether to execute constant folding for optimization
        export_params=True,
        training=torch.onnx.TrainingMode.EVAL,
        input_names=["input"],
    ...
    for i in range(args.batch_size):
        print_diffs(f"{i}", logits_py[0][i].detach().numpy(), logits_onnx[0][i], 5)
