...
async function main() {
  const response = await fetch(`${location.origin}/models.txt`);
  ...
  document
    ...
    .addEventListener("input", function (event) {
      document.getElementById("file_input").innerText =
  ...
  if (location.host.startsWith("localhost")) {
    ...
    const fileUrl = `${location.origin}/default.h5ad`; // Replace with actual file URL
    const fileName = "defaultt r.h5ad"; // Replace with desired file name
    ...
    try {
      ...
      // Update the label to show the file name
      document.getElementById("file_input").innerText = file.name;
    } catch (error) {
