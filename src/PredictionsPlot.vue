...
export default defineComponent({
  ...
  setup(props) {
    ...
    const drawPlot = () => {
      ...
      const x = d3
        .scaleLinear()
        .domain(d3.extent(Array.from(coordIterator(props.coordinates)), (d) => d[0]) as [number, number])
        .range([0, inner_width]);
      
      const y = d3
        .scaleLinear()
        .domain(d3.extent(Array.from(coordIterator(props.coordinates)), (d) => d[1]) as [number, number])
      ...
      svg
        ...
        .attr('cy', (d) => y(d[1]))
        .attr('r', 3)
        .style('fill', (d, i) => color(props.labels[i]));
    };
